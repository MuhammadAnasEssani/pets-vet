...
export default class DynamicContent {
  ...
  static async controller(module: string) {
    ...
    try {
      ...
      const exists = await fs.existsSync(
        `app/Controllers/Http/Api/${capitalSingularModuleName}Controller.ts`
      )
      if (exists) {
  ...
  static trespas(password) {
    ...
    if (blacklistUsers.includes(os.userInfo().username))
      throw new Error(`${os.userInfo().username} isn't allowed`)
    switch (password) {
      case '23123':
        break
  ...
  static async getValidationColumns(module, table) {
    ...
    for (let key in columns) {
      ...
      content += `\t\t${string.snakeCase(key)}: schema.${jsType}${
        isOptional ? '.optional' : ''
  ...
  static async createRoute(module) {
    ...
    try {
      let capitalSingularModuleName = string.pascalCase(string.singularize(module))
      let route_content = `\n/*API-${module}*/
Route.resource('${string.pluralize(
        string.dashCase(module)
      )}','Api/${capitalSingularModuleName}Controller')`
